...
export class MarketTicker {
	...
	deserialize(input: { symbol: string; currencies: any }): MarketTicker {
		const self: any = this;
		...
		for (const currency of CURRENCIES_LIST) {
			...
			if (input.currencies && input.currencies[currencyCode]) {
				...
				marketCurrency.marketCap =
					input.currencies[currencyCode].MKTCAP;
				...
				marketCurrency.change24h =
					input.currencies[currencyCode].CHANGEPCT24HOUR || null;
			}
...
export class MarketHistory {
		...
		for (const currency in input) {
			if (currency) {
				for (const data of input[currency]) {
					const date = new Date(data.time * 1000).setHours(
						0,
						0,
						0,
						0,
	...
	getLastWeekPrice(currencyCode: string): any {
		const dates = lodash(this.history[currencyCode])
			.keys()
			.takeRight(7)
			.value()
