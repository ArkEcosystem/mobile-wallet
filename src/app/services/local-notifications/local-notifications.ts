...
import { Transaction, UserSettings, Wallet } from "@/models/model";
	...
	watchTransactions(wallets: any) {
		for (const address in wallets) {
			if (address) {
				...
				client.transaction
					.list({ senderId: address, recipientId: address })
					.subscribe(response => {
						if (!response.success) {
							return;
						}
						...
						if (
							Number(response.count) > wallet.transactions.length
						) {
							...
							this.userDataProvider.updateWallet(
								w,
								wallet.profileId,
							);

							// Get only the new transaction and notify the user
							const newTransactions = lodash.drop(
								w.transactions,
								localLength,
							);
	...
	private notifyTransaction(transactions: Transaction[], wallet: any) {
		...
		for (const transaction of transactions) {
			...
			this.translateService
				.get([activityLabel, "WALLETS_PAGE.NEW_TRANSACTION"], {
					profile: wallet.profileName,
				})
				.subscribe((translation: string[]) => {
					const username = this.getAccountLabel(
						transaction.address,
						wallet.profileId,
					);
					const recipientLabel = this.getAccountLabel(
						transaction.getAppropriateAddress(),
						wallet.profileId,
					);

					const title = `💸 ${translation["WALLETS_PAGE.NEW_TRANSACTION"]}`;
					const text = `${username} ${translation[
						activityLabel
					].toLowerCase()} ${recipientLabel}`;
			...
			(acc, value: any, key) => {
				for (const address in value.wallets) {
					if (address) {
						...
						info.profileId = key;
						info.network = this.userDataProvider.getNetworkById(
							value.networkId,
						);
						acc[address] = info;
					}
